...
The primary uncertainty procedure is temporarily **under preregistration runtime review**. The previous draft used a 5,000-replicate patient-cluster bootstrap of paired repeat-1 out-of-fold predictions without model refitting after a synthetic workstation benchmark suggested that exact model-refit bootstrapping was infeasible. An independent reproduction found the same benchmark code to be hundreds of times faster. Before OSF registration, the workstation threading/runtime environment will be re-benchmarked using synthetic data only. If the original slow runtime is not reproduced under an explicit thread configuration, the refit-bootstrap interval will be restored before registration. No v2.1 predictive performance has been opened. Source patients are sampled with replacement, and all ICU rows from each sampled patient enter both prediction vectors with that patient's bootstrap multiplicity. Each replicate recomputes delta-AUROC. Child seeds are generated with numpy.random.SeedSequence(20260924).spawn(5000), and the reported interval is the two-sided 95% percentile interval.
