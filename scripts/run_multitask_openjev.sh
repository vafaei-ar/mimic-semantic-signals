...
.venv/bin/python src/22_run_open_jev_real_local.py \
  ...
  --output data/real_mimic_local/multitask_benchmark_v1/invasive_ventilation/open_jev_raw.jsonl \
  --device cuda \
  --progress-offset 0 \
  --progress-total "$TOTAL" \
  --progress-phase openjev_multitask

.venv/bin/python src/22_run_open_jev_real_local.py \
  --cases data/real_mimic_local/multitask_benchmark_v1/renal_replacement_therapy/cases.jsonl \
  --output data/real_mimic_local/multitask_benchmark_v1/renal_replacement_therapy/open_jev_raw.jsonl \
  --device cuda \
...
.venv/bin/python src/22_run_open_jev_real_local.py \
  ...
  --output data/real_mimic_local/multitask_benchmark_v1/icu_death/open_jev_raw.jsonl \
  --device cuda \
