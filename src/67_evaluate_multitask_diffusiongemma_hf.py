...
def load_diffusiongemma(path: Path) -> pd.DataFrame:
    ...
    if out.empty:
        raise RuntimeError(f"No successful DiffusionGemma semantic rows in {path}")
    return out
...
def repeat_diff(repeat_df: pd.DataFrame, a: str, b: str) -> dict:
    wide = repeat_df.pivot_table(index="repeat", columns="model", values="auroc").dropna(subset=[a, b])
...
def bootstrap_auc_diff(averaged: pd.DataFrame, a: str, b: str, seed: int, n_boot: int) -> dict:
...
def bootstrap_model_metrics(averaged: pd.DataFrame, model: str, seed: int, n_boot: int) -> dict:
...
def evaluate_one(
    ...
    df = features.merge(dg, on="case_id", how="inner")
    if len(df) != len(features):
        raise RuntimeError(
            f"{outcome}: expected {len(features)} complete DiffusionGemma rows but found {len(df)}"
        )
    ...
    excluded = {
        ...
        *categorical, *context_numeric, *dg_cols,
    }
    physiology = [
    ...
    models = {
        ...
        "diffusiongemma_8_scores_only": dg_cols,
        "context_plus_diffusiongemma": context + dg_cols,
        "structured_plus_diffusiongemma": context + physiology + dg_cols,
    }
    ...
    for repeat in range(repeats):
        ...
        for fold, (tr, te) in enumerate(cv.split(df, y, groups=groups)):
            ...
            update_progress(
                ...
                phase="diffusiongemma_multitask_evaluation",
    ...
    comparisons = {
        "diffusiongemma_semantics_only_minus_context": repeat_diff(
            repeat_df, "diffusiongemma_8_scores_only", "context_only"
        ),
        "structured_plus_diffusiongemma_minus_structured": repeat_diff(
            repeat_df, "structured_plus_diffusiongemma", "structured_only"
        ),
    }
    bootstrap_diffs = {
        "structured_plus_diffusiongemma_minus_structured": bootstrap_auc_diff(
            averaged,
            "structured_plus_diffusiongemma",
            "structured_only",
            seed + 500,
    ...
    return {
        ...
        "semantic_features": {"diffusiongemma": dg_cols},
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Evaluate frozen native-HF DiffusionGemma prompted semantic scores across multiple MIMIC outcomes."
    )
    ...
    report = {
        "analysis": "Frozen v1 multi-outcome native-HF DiffusionGemma semantic evaluation",
        ...
        "strict_semantics_only_definition": (
            "The 8-scores-only model contains exactly the eight DiffusionGemma prompted "
            "semantic scores and no hours-since-ICU, note category, dbsource, or physiology variables."
        ),
        "outcomes": results,
        "interpretation_guardrail": (
            "This is a robustness representation using generated support scores from native "
            "DiffusionGemma, scientifically distinct from Open-Jev/Laya noul probabilities. "
            "The matched cohorts do not represent population prevalence, so Brier scores are "
            "descriptive within the sampled design and are not population calibration estimates."
        ),
    }
