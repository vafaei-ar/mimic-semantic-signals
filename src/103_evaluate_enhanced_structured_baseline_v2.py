...
def bootstrap_two_models(df, n_boot, progress_base, total_progress, outcome):
    ...
    for b in range(n_boot):
        ...
        for name, p in predictions.items():
            ...
            per_model[name]["auroc"].append(auc)
            per_model[name]["auprc"].append(ap)
            per_model[name]["brier"].append(brier)
            per_model[name]["log_loss"].append(ll)
            per_model[name]["calibration_intercept"].append(intercept)
            per_model[name]["calibration_slope"].append(slope)
            ...
            for pt in THRESHOLDS:
                ...
                nb[name][key].append(float(tp / n_eff - (fp / n_eff) * (pt / (1 - pt))))
        else:
            for metric in diffs:
                diffs[metric].append(
                    float(current_metrics["nonlinear"][metric] - current_metrics["linear"][metric])
                )
            used += 1
        ...
        if (b + 1) % 25 == 0 or (b + 1) == n_boot:
            update_progress(
                ...
                phase="enhanced_structured_bootstrap",
    ...
    return {
        ...
        "models": {
            name: {
                "metrics_ci95": {k: _quantile_ci(v) for k, v in vals.items()},
                "decision_curve_model_net_benefit_ci95": {
                    k: _quantile_ci(v) for k, v in nb[name].items()
                },
            }
            for name, vals in per_model.items()
        },
        "paired_nonlinear_minus_linear_ci95": {
            k: _quantile_ci(v) for k, v in diffs.items()
        },
    }
...
def prepare_features(df):
    ...
    if len(feature_cols) != 34:
        raise RuntimeError(f"Expected 34 raw features, found {len(feature_cols)}: {feature_cols}")
    ...
    if len(x.columns) != 34:
        raise RuntimeError(f"Encoded feature count changed unexpectedly: {len(x.columns)}")
    return x, list(x.columns)


def main():
    ap = argparse.ArgumentParser(description="Evaluate frozen enhanced structured baseline v2.")
    ...
    if args.folds != 5:
        raise RuntimeError("v2 protocol freezes exactly 5 folds")
    ...
    for oi, outcome in enumerate(OUTCOMES):
        path = base / outcome / "enhanced_structured_features_v2_local.csv"
        ...
        split_df = df[["case_id", "subject_id"]].copy()
        fold_summaries = []
        ...
        for ri, seed in enumerate(REPEAT_SEEDS):
            ...
            for fold, (tr, te) in enumerate(cv.split(x, y, groups=groups), start=1):
                ...
                linear_model, nonlinear_model = build_models(seed + fold)

                linear_model.fit(x.iloc[tr], y[tr])
                nonlinear_model.fit(x.iloc[tr], y[tr])

                linear_preds[te, ri] = linear_model.predict_proba(x.iloc[te])[:, 1]
                nonlinear_preds[te, ri] = nonlinear_model.predict_proba(x.iloc[te])[:, 1]
                ...
                update_progress(
                    current=outcome_base_progress + ri * args.folds + fold,
                    total=total_progress,
                    phase="enhanced_structured_crossfit",
                    message=f"{outcome}: repeat {ri+1}/{repeats}, fold {fold}/{args.folds}",
                    unit="fold",
                )

            if np.any(fold_assignment < 1):
                raise RuntimeError(f"{outcome}: incomplete fold assignment in repeat {ri+1}")
            split_df[f"repeat_{ri+1}_fold"] = fold_assignment

            if np.isnan(linear_preds[:, ri]).any() or np.isnan(nonlinear_preds[:, ri]).any():
                raise RuntimeError(f"{outcome}: incomplete out-of-fold predictions in repeat {ri+1}")

            lm = basic_metrics(y, linear_preds[:, ri])
            nm = basic_metrics(y, nonlinear_preds[:, ri])
            repeat_metrics_full["linear"].append({"repeat": int(ri + 1), "seed": int(seed), **lm})
            repeat_metrics_full["nonlinear"].append({"repeat": int(ri + 1), "seed": int(seed), **nm})
        ...
        local_pred = df[["case_id", "subject_id", "icustay_id", "label"]].copy()
        ...
        pred_path = base / outcome / "enhanced_structured_predictions_v2_local.csv"
        split_path = base / outcome / "enhanced_structured_cv_splits_v2_local.csv"
        ...
        report["outcomes"][outcome] = {
            ...
            "models": {
                "linear": {
                    ...
                    "repeat_variability": summarize_repeat_metrics(repeat_metric_rows["linear"]),
                },
                "nonlinear": {
                    "averaged_oof_metrics": nonlinear_metrics,
                    "repeat_metrics": repeat_metrics_full["nonlinear"],
                    "repeat_variability": summarize_repeat_metrics(repeat_metric_rows["nonlinear"]),
                },
            },
            "bootstrap": boot,
    ...
    report["guardrails"] = [
        ...
        "Five repeated five-fold partitions are used and saved locally for later v2 narrative comparisons.",
        "No semantic, lexical, note-context, or dbsource predictor is used.",
        "Bootstrap intervals condition on repeat-averaged cross-fitted predictions and do not refit models.",
        "Row-level predictions and fold assignments remain local.",
    ]
