...
LANGUAGE_PATTERNS = {
    ...
    "icu_death": [
        ...
        r"\bCMO\b",
        r"\bwithdraw\w*\s+(?:care|support|treatment)\b",
        r"\bhospice\b",
        r"\bDNR\b",
        r"\bDNI\b",
...
def stable_case_id(outcome: str, icustay_id: int) -> str:
    raw = f"population_landmark12_v2|{outcome}|{int(icustay_id)}"
...
def load_icustays(root: Path) -> pd.DataFrame:
    ...
    d = lower_columns(
        pd.read_csv(
            ...
            in {"subject_id", "hadm_id", "icustay_id", "dbsource", "intime", "outtime"},
            low_memory=False,
        )
    )
...
def build_outcome(
    ...
    pop["event_time"] = pop["first_endpoint_time"]
    ...
    pop["case_id"] = [stable_case_id(outcome, x) for x in pop["icustay_id"]]
    if pop["case_id"].duplicated().any():
        raise RuntimeError(f"{outcome}: duplicate case ids")

    rx = compile_rx(LANGUAGE_PATTERNS[outcome])
...
def main() -> None:
    ...
    vent_proc = scan_procedure_events(root, VENT_ENDPOINT_PROCEDURE_IDS, "vent_endpoint")
    vent_chart = scan_chart_events(root, VENT_MV_DISQUAL_CHART_IDS, "vent_disqual_chart")
    vent_events = assign_events_to_icu(pd.concat([vent_proc, vent_chart], ignore_index=True), icu)
    ...
    vent_endpoint = first_time(vent_events, {"vent_endpoint"})
    vent_disqual = first_time(vent_events, {"vent_endpoint", "vent_disqual_chart"})
    ...
    report = {
        "analysis": "Corrected 12-hour landmark cohort build v2",
